...
const categoryColors = {
  date: 'from-blush-pink to-rose-gold',
  adventure: 'from-mint-mist to-peach-puff',
  everyday: 'from-honey-amber to-lavender-haze',
  special: 'from-rose-gold to-blush-pink'
}
...
function MemoryCard({ memory, index, onToggleFavorite }: MemoryCardProps) {
  ...
  return (
    ...
    >
      <motion.div
        className="relative bg-white rounded-2xl overflow-hidden shadow-lg border border-rose-gold/20 cursor-pointer group"
      ...
      >
        ...
        <div className="relative overflow-hidden">
          ...
          {memory.hasMusic && (
            <motion.div
              ...
              transition={{ duration: 0.3 }}
            >
              <Music size={14} className="text-blush-pink" />
            </motion.div>
          )}
          ...
          <motion.div
            ...
            transition={{ duration: 0.3 }}
          >
            <p className="font-caveat text-signature mb-1">
              {memory.note}
            </p>
        ...
        {/* Card content */}
        <div className="p-4">
          <h3 className="font-playfair text-lg font-medium text-blush-pink mb-2">
            {memory.title}
          </h3>
              ...
              >
                <Heart 
                  ...
                  className={`transition-colors duration-200 ${
                    memory.isFavorited 
                      ? 'text-blush-pink fill-blush-pink' 
                      : 'text-gray-400 group-hover/heart:text-blush-pink'
                  }`}
                />
              </motion.div>
        ...
        {memory.isFavorited && (
          <motion.div
            className="absolute -top-1 -right-1 text-blush-pink text-sm"
            animate={{ 
              scale: [1, 1.2, 1],
              rotate: [0, 180, 360]
            }}
...
export default function Memories() {
  ...
  return (
    <div className="min-h-screen pt-8 pb-24 px-4 relative overflow-hidden">
      ...
      <motion.div
        className="absolute top-40 left-8 text-xl opacity-30 animate-float-delayed"
        initial={{ opacity: 0 }}
        animate={{ opacity: 0.3 }}
      ...
      >
        ...
        >
          <div className="text-center">
            <div className="font-playfair text-2xl text-blush-pink">{memoryList.length}</div>
            <div className="font-caveat text-sm text-peach-puff">Memories</div>
          </div>
          <div className="text-center">
            <div className="font-playfair text-2xl text-rose-gold">{favoriteCount}</div>
            <div className="font-caveat text-sm text-peach-puff">Favorites</div>
          </div>
        </motion.div>
          ...
          {['all', 'date', 'adventure', 'everyday', 'special'].map((category) => (
            <motion.button
              ...
              className={`px-4 py-2 rounded-full font-caveat text-sm transition-all duration-200 ${
                filter === category
                  ? 'bg-gradient-romance text-white shadow-heart-glow'
                  : 'bg-white/60 text-gray-600 hover:bg-white/80 border border-rose-gold/20'
              }`}
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
            >
      ...
      {filteredMemories.length === 0 && (
        ...
        >
          ...
          <p className="font-caveat text-signature text-peach-puff">
