...
const difficultyColors = {
  easy: 'from-muted to-secondary',
  medium: 'from-secondary to-primary',
  hard: 'from-accent to-primary',
  ...
  const handleComplete = () => {
    ...
    if (quest.status === 'completed') {
      toast({
        ...
        description: quest.completedMessage || `You've earned the ${quest.reward}`
      });
      return;
    }
    ...
    >
      <motion.div
        className={`relative p-6 rounded-2xl border-2 transition-all duration-300 cursor-pointer ${
          quest.status === 'locked' 
            ...
            ? 'border-primary/40 bg-gradient-to-br from-white/90 to-primary/20 shadow-lg'
            : 'border-primary/30 bg-white/80 hover:shadow-lg hover:border-primary/50'
        }`}
        animate={{
      ...
      >
        ...
        <div>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-2">
              ...
              <span className="font-caveat text-lg text-secondary">
                {quest.reward}
              </span>
            </div>
        ...
        {isHovered && quest.status !== 'locked' && (
          <motion.div
            className="absolute inset-0 bg-gradient-to-r from-primary/10 to-secondary/10 rounded-2xl pointer-events-none"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          />
  ...
  return (
    <div className="min-h-screen pt-8 pb-24 px-4 relative overflow-hidden">
      ...
      >
        ...
        {/* Progress indicator */}
        <motion.div
          className="bg-white/60 backdrop-blur-sm rounded-full p-4 max-w-sm mx-auto shadow-lg border border-primary/20"
          initial={{ scale: 0 }}
          animate={{ scale: 1 }}
          transition={{ delay: 0.5, type: "spring" }}
        >
          <div className="flex items-center justify-center space-x-3">
            <span className="font-caveat text-lg text-primary">
              Progress:
            </span>
